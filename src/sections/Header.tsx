      ...
      >
        <nav className="mt-20 flex flex-col">
          {navItems.map(({ label, href }) => (
            <a
              className="text-stone-200 border-t border-stone-800 isolate last:border-b py-8 relative group/nav-item"
              href={href}
              key={label}
              onClick={handleClickMobileNavItem}
            >
              <div className="container !max-w-full flex items-center justify-between">
                <span className="text-3xl group-hover/nav-item:pl-4 text-[#EC5000] transition-all duration-500">{label}</span>
                <svg
